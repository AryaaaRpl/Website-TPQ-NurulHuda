...
class HomeController {
  public function index() { $items=Content::where('status','published')->orderBy('sort_order')->get()->groupBy('type'); return view('home',['gallery'=>$items->get('gallery',collect()),'extras'=>$items->get('extracurricular',collect()),'facilities'=>$items->get('facility',collect()),'testimonials'=>Testimonial::where('status','published')->orderBy('sort_order')->get(),'setting'=>SiteSetting::first()]); }
 ...
 public function ekskul() { $extras=Content::where('type','extracurricular')->where('status','published')->orderBy('sort_order')->get(); return view('ekskul',['extras'=>$extras]); }
  ...
  if ($request->isMethod('post')) {
   ...
   return back()->with('registration_success'," Yeay! Pendaftaran berhasil dikirim. Nomor pendaftaran: {$a->registration_number}");
  }
  $facilities=Content::where('type','facility')->where('status','published')->orderBy('sort_order')->get()->take(3);
  return view('ppdb',['facilities'=>$facilities,'setting'=>SiteSetting::first()]);
 }
}
